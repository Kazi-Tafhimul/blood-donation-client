...
import { imageUploader } from "@/lib/imageUpload"; // Ensure this path is correct
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Handle image upload via helper
      if (avatarFile) {
        const imageData = await imageUploader(avatarFile);
            ...
            <Fieldset.Group className="grid grid-cols-1 gap-5 w-full">
                  ...
                  <Select.Popover>
                    <ListBox>
                      {["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"].map((bg) => (
                        <ListBox.Item id={bg} key={bg}>{bg}</ListBox.Item>
                      ))}
                    </ListBox>
                  </Select.Popover>
                </Select>

                <Select isRequired name="district" placeholder="Select District" value={selectedDistrict} onChange={(key) => setSelectedDistrict(String(key))}>
                  <Label className="text-[14px] font-bold text-zinc-800">District</Label>
                  <Select.Trigger><Select.Value /><Select.Indicator /></Select.Trigger>
                  <Select.Popover>
                    <ListBox>
                      {districtsList.map((dist) => (
                        <ListBox.Item id={dist.name} key={dist.id}>{dist.name}</ListBox.Item>
                      ))}
                    </ListBox>
                  </Select.Popover>
                </Select>

                <Select isRequired name="upazila" placeholder="Select Upazila" isDisabled={filteredUpazilas.length === 0}>
                  <Label className="text-[14px] font-bold text-zinc-800">Upazila</Label>
                  <Select.Trigger><Select.Value /><Select.Indicator /></Select.Trigger>
                  <Select.Popover>
                    <ListBox>
                      {filteredUpazilas.map((upz) => (
                        <ListBox.Item id={upz.name} key={upz.id}>{upz.name}</ListBox.Item>
                      ))}
                    </ListBox>
                  </Select.Popover>
                </Select>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 w-full">
                <TextField isRequired name="password">
                  ...
                  <div className="relative w-full">
                    <Input type={showPassword ? "text" : "password"} placeholder="••••••••" className="w-full bg-zinc-50/50 rounded-xl" />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-zinc-600 z-20">
              ...
              <div className="flex flex-col gap-2 w-full">
                ...
                <RadioGroup name="role" defaultValue="donor" orientation="horizontal">
                  <Radio value="donor"><Radio.Content><Radio.Control><Radio.Indicator /></Radio.Control><span className="text-sm font-medium text-zinc-700 ml-1">Donor</span></Radio.Content></Radio>
                  <Radio value="volunteer"><Radio.Content><Radio.Control><Radio.Indicator /></Radio.Control><span className="text-sm font-medium text-zinc-700 ml-1">Volunteer</span></Radio.Content></Radio>
                </RadioGroup>
              </div>
            </Fieldset.Group>

            <Button type="submit" disabled={isSubmitting} className="w-full bg-[#D62828] hover:bg-[#b21e1e] disabled:bg-zinc-400 text-white text-[15px] font-bold py-3.5 rounded-xl transition-all flex items-center justify-center gap-2">
              {isSubmitting ? <><Loader2 className="h-4 w-4 animate-spin" /> Creating Profile...</> : "Sign Up"}
            </Button>

            <p className="text-center text-sm text-zinc-500 font-medium pt-2">
              Already have an account? <Link href="/login" className="text-[#14B8A6] hover:underline font-bold">Sign In</Link>
            </p>
          </Fieldset>
        </Form>
      </div>
